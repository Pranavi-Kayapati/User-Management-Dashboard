...
const UserList = () => {
  ...
  const { users, isLoading, isError, error } = useSelector((state) => state);
  ...
  const buttonText = useBreakpointValue({ base: "", md: "Add user" });
  useEffect(() => {
    dispatch(getUser());
  }, [dispatch]);
  ...
  const handleFormSubmit = (formData) => {
    if (selectedUser) {
      dispatch(updateUser(formData, selectedUser.id));
    } else {
      let newUser = {
        id: users.length + 1,
        ...formData,
  ...
  return (
    <Box borderWidth="0px" borderRadius="md" overflow="hidden">
      <Box p={4} bg="white" boxShadow="md" borderRadius="md">
        <Flex align="center" mb={4} gap={{ base: 2, md: 4 }}>
          <Box flex="1">
            <Heading size="lg">Users</Heading>
            <Text color="gray.500">Find all department users here</Text>
          </Box>
          ...
          <Tr>
            ...
            <Th onClick={() => handleSortChange("department")}>
              <Flex align={"center"}>
                ...
                <span>{renderSortIcon("department")}</span>
              </Flex>
            </Th>
            <Th></Th>
          </Tr>
          ...
          {isLoading ? (
            ...
            paginatedUsers.map((user) => (
              <Tr key={user.id} p={2}>
                ...
                <Td>{user.name}</Td>
                <Td>{user.email}</Td>
                <Td>{user.department || "Development"}</Td>
                <Td>
                  <Popover placement="left-start">
                    <PopoverTrigger>
                      <Button variant={"text"}>
